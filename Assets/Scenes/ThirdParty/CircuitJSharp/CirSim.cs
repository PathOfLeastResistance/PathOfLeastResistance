...
public class CirSim
{
    ...
    const int MODE_ADD_ELM = 0;
    const int MODE_DRAG_ALL = 1;
    const int MODE_DRAG_ROW = 2;
    const int MODE_DRAG_COLUMN = 3;
    const int MODE_DRAG_SELECTED = 4;
    const int MODE_DRAG_POST = 5;
    const int MODE_SELECT = 6;
    const int MODE_DRAG_SPLITTER = 7;
...
//    String baseURL = "http://www.falstad.com/circuit/";
    ...
    bool getOptionFromStorage(String key, bool val)
    {
        Storage stor = Storage.getLocalStorageIfSupported();
        if (stor == null)
            return val;
...
// the other)
    bool calcWireInfo()
    {
        ...
        for (i = 0; i != wireInfoList.size(); i++)
        {
            ...
            bool isReady0 = true, isReady1 = !(wire instanceof GroundElm);
            ...
            for (j = 0; j != cn1.links.size(); j++)
            {
                ...
                instanceof LabeledNodeElm && wire instanceof LabeledNodeElm &&
...
void findUnconnectedNodes() {
    ...
    boolean closure[] = new bool[nodeList.size()];
    bool changed = true;
...
bool canDelayWireProcessing()
{
    ...
    for (i = 0; i != elmList.size(); i++)
        if (getElm(i)
    instanceof ScopeElm && ((ScopeElm)getElm(i)).elmScope.viewingWire())
    return false;
    return true;
}
...
bool canCombineScope(int s)
{
    return scopeCount >= 2;
}

bool canUnstackScope(int s)
{
    if (scopeCount < 2)
        return false;
    if (s == 0)
...
public void importCircuitFromText(String circuitText, bool subcircuitsOnly)
{
...
void processSetupList(byte b
[], final boolean openDefault) {
    int len = b.length;
...
void dragPost(int x, int y, bool all)
{
    if (draggingPost == -1)
    {
        draggingPost =
            (Graphics.distanceSq(mouseElm.x, mouseElm.y, x, y) >
             Graphics.distanceSq(mouseElm.x2, mouseElm.y2, x, y))
                ? 1
                : 0;
    }
...
void selectArea(int x, int y, bool add)
{
...
public void onMouseWheel(MouseWheelEvent e)
{
    ...
    bool zoomOnly = System.currentTimeMillis() < zoomTime + 1000;
...
void scrollValues(int x, int y, int deltay)
{
    if (mouseElm != null && !dialogIsShowing() && scopeSelected == -1)
        if (mouseElm
    instanceof ResistorElm || mouseElm instanceof CapacitorElm || mouseElm instanceof InductorElm) {
...
static bool lu_factor(double a
[]
[], int n,  int ipvt[]) {
    int i, j, k;
...
static void lu_solve(double a
[]
[], int n,  int ipvt[], double b[]) {
    int i;
...
bool loadedCanvas2SVG = false;

bool initializeSVGScriptIfNecessary(final String followupAction)
{
...
void drawCircuitInContext(Context2d context, int type, Rectangle bounds, int w, int h)
{
    ...
    bool p = printableCheckItem.getState();
    bool c = dotsCheckItem.getState();
    bool print = (type == CAC_PRINT);
    if (print)
...
//	    String models = "";
    ...
    boolean used[] = new bool[nodeList.size()];
    boolean extnodes[] = new bool[nodeList.size()];
...
static void invertMatrix(double a
[]
[], int n) {
    int ipvt[] = new int[n];
    lu_factor(a, n, ipvt);
...
native JsArray<JavaScriptObject>
getJSArray() /*-{ return []; }-*/;
